...
type InternalSelectConfig = Omit<YdbSelectConfig, 'table'> & { table?: unknown }
	...
	implements YdbSetOperatorSource
{
	...
	readonly #session: YdbSession | undefined
	readonly #dialect: YdbDialect
	readonly #config: InternalSelectConfig
	readonly #isPartialSelect: boolean
	#joinsNotNullableMap: Record<string, boolean> = {}
	#tableName: string | undefined
	#usedInSetOperation = false
	...
	#requireTable(): unknown | undefined {
	...
	#requireConfigWithTable(): SelectConfigWithTable {
		let table = this.#requireTable()
	...
	#markUsedInSetOperation(): void {
		this.#usedInSetOperation = true
	}

	#shouldUseSelectionAliases(): boolean {
		return (
	...
	#getSelectionAliases(fields = this.#getOrderedFields()): string[] | undefined {
		return this.#shouldUseSelectionAliases()
			? this.#dialect.getSelectionAliases(fields)
			: undefined
	}

	#getTargetConfigForTailClauses(): YdbSetOperatorConfig | InternalSelectConfig {
		return this.#config.setOperators[this.#config.setOperators.length - 1] ?? this.#config
	}

	#createJoin(joinType: YdbJoinType) {
	...
	#applyWithoutToFields(values: readonly (string | SQLWrapper)[]): void {
		if (Object.keys(this.#config.fields).length === 0) {
			return
		}

		let fieldEntries = Object.entries(this.#config.fields)
		let removedKeys = new Set<string>()
		let removedColumnNames = new Set<string>()
	...
	#normalizeWithout(values: readonly (string | SQLWrapper)[]): SQLWrapper[] {
		if (values.length === 0) {
	...
	#setFlatten(mode: YdbFlattenMode, expressions: readonly SQLWrapper[] = []): this {
	...
	#createSetOperator(type: 'union' | 'intersect' | 'except', isAll: boolean) {
		return (
			rightSelection:
				| YdbSetOperatorSource
			...
			if (rightSelect instanceof YdbSelectBuilder) {
				rightSelect.#markUsedInSetOperation()
			}
	...
	groupBy(...columns: SQLWrapper[] | [SelectionCallback]): this {
		if (typeof columns[0] === 'function') {
			let groupBy = columns[0](createSelectionProxy(this.#config.fields, 'alias'))
	...
	orderBy(...columns: SQLWrapper[] | [SelectionCallback]): this {
		let target = this.#getTargetConfigForTailClauses()
		...
		if (typeof columns[0] === 'function') {
			let orderBy = columns[0](createSelectionProxy(this.#config.fields, 'alias'))
			;(target as YdbSetOperatorConfig | InternalSelectConfig).orderBy = Array.isArray(
				orderBy
			)
				? orderBy
				: [orderBy]
		...
		;(target as YdbSetOperatorConfig | InternalSelectConfig).orderBy = columns as SQLWrapper[]
		return this
	}
	...
	assumeOrderBy(...columns: (string | SQLWrapper)[] | [(string | SQLWrapper)[]]): this {
		...
		let resolved = Array.isArray(columns[0])
	...
	limit(limit: number): this {
		let target = this.#getTargetConfigForTailClauses()
		;(target as YdbSetOperatorConfig | InternalSelectConfig).limit = normalizeCountValue(
			limit,
			'limit'
		)
		return this
	...
	offset(offset: number): this {
		let target = this.#getTargetConfigForTailClauses()
		;(target as YdbSetOperatorConfig | InternalSelectConfig).offset = normalizeCountValue(
			offset,
			'offset'
		)
		return this
	...
	uniqueDistinct(...hints: YdbUniqueDistinctHint[] | [YdbUniqueDistinctHint[]]): this {
		let resolved = Array.isArray(hints[0])
	...
	distinctOn(...values: SQLWrapper[] | [SQLWrapper[]]): this {
		...
		let resolved = Array.isArray(values[0])
	...
	addSetOperators(setOperators: YdbSetOperatorConfig[]): this {
		...
		for (let setOperator of setOperators) {
			if (setOperator.rightSelect instanceof YdbSelectBuilder) {
				setOperator.rightSelect.#markUsedInSetOperation()
			}
		}
	...
	getSQL(selectionAliases?: string[]): SQL {
		let config = this.#requireConfigWithTable()
		let fieldsFlat = orderSelectedFields(config.fields)
		let aliases = selectionAliases ?? this.#getSelectionAliases(fieldsFlat)
	...
	prepare(name?: string) {
		...
		let orderedFields = this.#getOrderedFields()
		let joinsNotNullableMap =
			Object.keys(this.#joinsNotNullableMap).length > 0
				? this.#joinsNotNullableMap
				: undefined
		let resultMapper = (rows: unknown[][]) =>
