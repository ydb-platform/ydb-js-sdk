...
Topic reader/writer переподнимают соединение при сбоях и пересоздают очередь команд. Переподнимают автоматически (экспоненциальная задержка + jitter) и по умолчанию неограниченно — ждут сервер/топик; передайте `recoveryWindowMs`, чтобы задать таймаут, и `retryOnSchemeError`, чтобы дождаться ещё не созданного топика. Обеспечивайте идемпотентность продюсера через `producer + seqNo`.
