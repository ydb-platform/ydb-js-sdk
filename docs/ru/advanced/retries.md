...
Topic reader/writer переподнимают соединение при сбоях и пересоздают очередь команд. Writer переподнимает соединение автоматически (экспоненциальная задержка + jitter, ограничено `recoveryWindowMs`); обеспечивайте идемпотентность продюсера через `producer + seqNo`.
