...
beforeEach(async (ctx) => {
	...
	await sessionA.semaphore(electionName).create({ limit: 1 }, ctx.signal)
...
test('returns null for a vacant pre-created semaphore', async (tc) => {
	...
	let info = await election.leader(tc.signal)

	expect(info).toBeNull()
})

test('campaign becomes leader', async (tc) => {
	let election = sessionA.election(electionName)
	let payload = Buffer.from('candidate-A')

	await using _leadership = await election.campaign(payload, tc.signal)
...
test('resign releases leadership', async (tc) => {
	let election = sessionA.election(electionName)

	let leadership = await election.campaign(Buffer.from('A'), tc.signal)
	await leadership.resign(tc.signal)
...
test('async dispose resigns', async (tc) => {
	let election = sessionA.election(electionName)

	{
		await using _leadership = await election.campaign(Buffer.from('A'), tc.signal)
	}

	// After dispose the semaphore is free — a second campaign must succeed immediately
	await using leadership2 = await election.campaign(Buffer.from('A2'), tc.signal)

	expect(leadership2.signal.aborted).toBe(false)
})

test('proclaim updates semaphore data', async (tc) => {
	...
	await using _leadership = await election.campaign(Buffer.from('initial'), tc.signal)

	await _leadership.proclaim(Buffer.from('updated'), tc.signal)

	// Read the semaphore's top-level data field — that is what proclaim writes to.
	let description = await sessionA.semaphore(electionName).describe({}, tc.signal)

	expect(Buffer.from(description.data)).toEqual(Buffer.from('updated'))
})

test('second campaign waits until first leader resigns', async (tc) => {
	...
	await using leadershipA = await electionA.campaign(Buffer.from('A'), tc.signal)

	// Session B starts campaigning — it blocks until A resigns
	let campaignB = electionB.campaign(Buffer.from('B'), tc.signal)
	...
	let info = await electionB.leader(tc.signal)

	expect(info).not.toBeNull()
	expect(Buffer.from(info!.data)).toEqual(Buffer.from('B'))
})

test('observe detects when a leader appears', async (tc) => {
	...
	let observing = (async () => {
		for await (let state of sessionA
			...
			.observe(AbortSignal.any([ctrl.signal, tc.signal]))) {
	...
	await using _leadership = await sessionB
		.election(electionName)
		.campaign(Buffer.from('leader-B'), tc.signal)
...
test('observe sets isMe true when the observing session is the leader', async (tc) => {
	let election = sessionA.election(electionName)

	// Campaign first so the first watch snapshot already has an owner
	await using _leadership = await election.campaign(Buffer.from('leader-A'), tc.signal)

	let firstState: { isMe: boolean; data: Buffer } | undefined

	for await (let state of election.observe(tc.signal)) {
...
test('observe tracks leader change and no-leader transition', async (tc) => {
	...
	await using sessionB = await client.createSession(testNodePath, {}, tc.signal)
	await using sessionC = await client.createSession(testNodePath, {}, tc.signal)

	// A becomes leader before observation starts so the first snapshot is non-empty
	await using leadershipA = await sessionA
		.election(electionName)
		.campaign(Buffer.from('A'), tc.signal)

	let states: Array<{ data: Buffer; isMe: boolean }> = []

	let observing = (async () => {
		for await (let state of sessionB.election(electionName).observe(tc.signal)) {
	...
	await using _leadershipC = await sessionC
		.election(electionName)
		.campaign(Buffer.from('C'), tc.signal)
...
test('leadership signal aborts when leader resigns', async (tc) => {
	let election = sessionA.election(electionName)
...
test('leader state signal aborts when the leader changes', async (tc) => {
...
test('leadership.signal.reason is LeaseReleasedError after resign', async (tc) => {
	let election = sessionA.election(electionName)
...
test('double resign is idempotent', async (tc) => {
	let election = sessionA.election(electionName)

	let leadership = await election.campaign(Buffer.from('A'), tc.signal)
	await leadership.resign(tc.signal)
	await expect(leadership.resign(tc.signal)).resolves.toBeUndefined()
})

test('N sessions race for leadership — exactly one wins', async (tc) => {
	let N = 4
	...
	try {
		// All sessions campaign simultaneously with short timeout so N-1 losers
		// reject quickly instead of hanging until the test's own timeout.
		...
		for (let r of winners) {
			// oxlint-disable-next-line no-await-in-loop
			await (r as PromiseFulfilledResult<any>).value.resign(tc.signal)
		}
	} finally {
