...
export let _on_stop_partition_session_request =
	async function on_stop_partition_session_request(
		ctx: {
		...
		assert.ok(
			...
			'stopPartitionSessionRequest must have partitionSessionId'
		)
		...
		if (!partitionSession) {
			dbg.log(
				'error: stopPartitionSessionRequest for unknown partitionSessionId=%s',
				input.partitionSessionId
			)
			return
		}
		...
		if (!input.graceful) {
			dbg.log(
				'stop partition session %s without graceful stop',
			...
			for (let part of ctx.buffer) {
				...
				while (i < part.partitionData.length) {
					if (
						part.partitionData[i]!.partitionSessionId ===
						partitionSession.partitionSessionId
					) {
		...
		if (
			ctx.pendingCommits &&
			ctx.pendingCommits.has(partitionSession.partitionSessionId)
		) {
			await Promise.race([
				Promise.all(
