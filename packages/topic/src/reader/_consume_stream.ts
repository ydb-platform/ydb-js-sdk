		...
		if (state.pendingCommits.size) {
			...
			for (let [
				...
				for (let commit of pendingCommits) {
					commit.reject(
						new Error(
							`Pending commit for partition session ${partitionSessionId} rejected before connecting to the stream`
						)
					)
				}
			}
				...
				void _on_commit_offset_response(
					{
						pendingCommits: state.pendingCommits,
						partitionSessions: state.partitionSessions,
						...(state.options.onCommittedOffset && {
							onCommittedOffset: state.options.onCommittedOffset,
						}),
					},
					chunk.serverMessage.value
				)
