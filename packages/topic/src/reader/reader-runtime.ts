...
import { ReaderTransport } from './transport.js'
...
let DEFAULT_GRACEFUL_SHUTDOWN_MS = 30_000
let DEFAULT_UPDATE_TOKEN_MS = 60_000
...
let timerEvent = function timerEvent(which: TimerName, partitionId?: bigint): ReaderEvent {
	switch (which) {
		case 'start_timeout':
		...
		case 'partition_reassign_gc':
			return { type: 'reader.timer.partition_reassign_gc', partitionId: partitionId! }
	}
}
...
// Timer key — partition_reassign_gc is per-partition, so it must not collide.
let timerKey = function timerKey(which: TimerName, partitionId?: bigint): string {
	return partitionId === undefined ? which : `${which}:${partitionId}`
}
...
export function createReaderRuntime(driver: Driver, options: TopicReaderOptions): ReaderRuntime {
	...
	let env: ReaderEnv = {
		...
		updateTokenIntervalMs: options.updateTokenIntervalMs ?? DEFAULT_UPDATE_TOKEN_MS,
		gracefulShutdownTimeoutMs: DEFAULT_GRACEFUL_SHUTDOWN_MS,
	...
	>({
		...
		effects: {
			...
			'reader.effect.transport.send_update_token': (fullCtx) => {
			...
			'reader.effect.partition.start_ack': async (fullCtx, effect) => {
				...
				// The partition may have been stopped or reassigned while the async
				// callback ran — don't answer for a session that is no longer current.
				let current = fullCtx.partitions.get(effect.partitionId)
				if (!current || current.sessionId !== effect.partitionSessionId) {
					return
				}

				// Omit read_offset/commit_offset unless the callback set them: the server
			...
			'reader.effect.timer.schedule': (fullCtx, effect, runtime) => {
				let { which, partitionId } = effect
				let key = timerKey(which, partitionId)
				// The recovery window is armed once per reconnect saga.
				if (which === 'recovery_window' && fullCtx.timers.has(key)) {
					return
				}
				clearTimerByKey(fullCtx, key)

				let repeating = which === 'update_token'
				let delay = delayFor(fullCtx, which)
				let event = timerEvent(which, partitionId)
			...
			'reader.effect.timer.clear': (fullCtx, effect) => {
				clearTimerByKey(fullCtx, timerKey(effect.which, effect.partitionId))
			},

			'reader.effect.finalize': (fullCtx, effect, runtime) => {
				...
				// Seal after this drain so buffered outputs (reader.closed / rejects) are
				// delivered first.
