...
let applyWriterEffect = function applyWriterEffect(sim: Sim, effect: WriterEffect): void {
	switch (effect.type) {
		case 'writer.effect.transport.connect':
			...
			break
		case 'writer.effect.transport.send_batch':
			...
			break
		case 'writer.effect.transport.send_update_token':
			if (sim.streamOpen && !sim.initPending) {
...
let checkInvariants = function checkInvariants(sim: Sim, where: string): void {
	...
	// authoritative — i.e. in the live, write-accepting states. During `closing`
	// the FSM deliberately stops reporting acks and a pending flush is rejected on
	// close (not resolved), and the facade resets #bufferedBytes to 0 on close; so
	// asserting either there would flag by-design behavior, not a defect.
		...
		if (sim.streamOpen && sim.initPending) {
			actions.push({
				...
				run: () => {
					...
					// get_last_seq_no is false (proven in writer-protocol.test.ts). When
