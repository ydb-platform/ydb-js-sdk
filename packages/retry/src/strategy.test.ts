...
test('applies fixed strategy with zero delay', async () => {
	...
	let delay = strategy({ attempt: 5, error: new Error('test') }, defaultRetryConfig)
	expect(delay).eq(0)
})
...
test('applies linear strategy with zero base', async () => {
	...
	delay = strategy({ attempt: 5, error: new Error('test') }, defaultRetryConfig)
	expect(delay).eq(0)
})
...
test('handles random strategy edge cases', async () => {
	...
	let delay2 = strategy2({ attempt: 0, error: new Error('test') }, defaultRetryConfig)
...
test('handles backoff strategy edge cases', async () => {
	...
	let delay2 = strategy2({ attempt: 1, error: new Error('test') }, defaultRetryConfig)
	expect(delay2).eq(0)
})
...
test('handles large attempt numbers correctly', async () => {
	...
	expect(fixedStrategy({ attempt: largeAttempt, error: new Error('test') }, defaultRetryConfig)).eq(100)

	let linearStrategy = strategies.linear(10)
	expect(linearStrategy({ attempt: largeAttempt, error: new Error('test') }, defaultRetryConfig)).eq(10000)

	// Exponential should handle large numbers without throwing
	let exponentialStrategy = strategies.exponential(1)
	let expResult = exponentialStrategy({ attempt: largeAttempt, error: new Error('test') }, defaultRetryConfig)
...
test('respects backoff strategy max limit properly', async () => {
	...
	expect(strategy({ attempt: 0, error: new Error('test') }, defaultRetryConfig)).eq(100)  // 2^0 * 100 = 100
	expect(strategy({ attempt: 1, error: new Error('test') }, defaultRetryConfig)).eq(200)  // 2^1 * 100 = 200
	expect(strategy({ attempt: 2, error: new Error('test') }, defaultRetryConfig)).eq(400)  // 2^2 * 100 = 400
	expect(strategy({ attempt: 3, error: new Error('test') }, defaultRetryConfig)).eq(500)  // 2^3 * 100 = 800, capped at 500
	expect(strategy({ attempt: 10, error: new Error('test') }, defaultRetryConfig)).eq(500) // Still capped at 500
})
...
test('handles negative delays in combine strategy', async () => {
	...
	let result = combined({ attempt: 0, error: new Error('test') }, defaultRetryConfig)
