...
function parseConnectionString(cs: string): {
	...
	if (!m || m[1] === undefined || m[2] === undefined) return { host: '', port: 0, database: '' }
...
function getSpanByName(
	...
	return attrs !== undefined ? { name: s.name, attributes: attrs } : { name: s.name }
}

beforeAll(async () => {
	const connectionString = inject('connectionString')
	driver = new Driver(connectionString, {
		'ydb.sdk.enable_discovery': false,
		...withTracing(connectionString, createOpenTelemetryTracer()),
	})
	await driver.ready()
})
...
test('rollback emits ydb.Rollback span', async () => {
	...
	assertCommonDbAttributes(rollbackSpan, parseConnectionString(inject('connectionString')))
})
...
test('executeQuery error sets error status and error.type on span', async () => {
	...
	expect(errorSpan == null || errorSpan.attributes?.['error.type'] != null).toBe(true)
})
