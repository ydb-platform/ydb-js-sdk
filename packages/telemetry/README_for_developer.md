...
diagnostics_channel.channel('tracing:ydb:query.transaction:start').bindStore(als, (ctx: any) => {
...
After this, `tx`select 1``inside the callback finds`ydb.Transaction`via`context.active()`and creates`ydb.Query.execute` as its child without any
...
execCh.subscribe({
  start(ctx) {
    spans.set(ctx, tracer.startSpan('ydb.Query.execute'))
  },
  error(ctx) {
    spans.get(ctx)?.recordException(ctx.error as Error)
  },
  asyncEnd(ctx) {
    spans.get(ctx)?.end()
  },
...
meter
  .createObservableUpDownCounter('ydb.pool.connections.active')
...
dc.subscribe('ydb:pool.connection.pessimized', () => {
  active--
  pessimized++
})
dc.subscribe('ydb:pool.connection.unpessimized', () => {
  active++
  pessimized--
})
```
...
export function safeSubscribe(name: string, fn: (msg: any) => void): () => void {
  let safe = (msg: unknown) => {
    try {
      ...
      dbg.log('subscriber for %s threw: %O', name, err)
    }
  }
    ...
    Object.entries(handlers).map(([k, fn]) => [
      ...
      (ctx: T) => {
        try {
          ...
          dbg.log('%s.%s threw: %O', name, k, err)
        }
      },
    ])
...
test('register() twice does not double-emit spans', async () => {
  ...
  register()
  await sql`select 1`
  expect(exporter.getFinishedSpans().filter((s) => s.name === 'ydb.Query.execute')).toHaveLength(1)
})
```
