...
import {
	ListFederationDatabasesRequestSchema,
	ListFederationDatabasesResponseSchema,
} from './protos/ydb_federation_discovery_pb.js'
	...
	listFederationDatabases: {
		...
		requestSerialize: (
			message: MessageInitShape<
				typeof ListFederationDatabasesRequestSchema
			>
		) =>
			toBinary(
				ListFederationDatabasesRequestSchema,
				create(ListFederationDatabasesRequestSchema, message)
			),
		requestDeserialize: (bytes: Uint8Array) =>
			fromBinary(ListFederationDatabasesRequestSchema, bytes),
		responseStream: false,
		responseSerialize: (
			message: MessageInitShape<
				typeof ListFederationDatabasesResponseSchema
			>
		) =>
			toBinary(
				ListFederationDatabasesResponseSchema,
				create(ListFederationDatabasesResponseSchema, message)
			),
		responseDeserialize: (bytes: Uint8Array) =>
			fromBinary(ListFederationDatabasesResponseSchema, bytes),
		options: {},
	},
...
//@ts-expect-error
FederationDiscoveryServiceDefinition['fullName'] =
